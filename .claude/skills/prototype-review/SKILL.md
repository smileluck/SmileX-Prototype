...
```
/prototype-review create <需求文件路径>                    # 根据需求文档创建新原型（含 SRS + 手册）
...
/prototype-review <slug> --srs                             # 为已有项目生成/更新 SRS
/prototype-review <slug> --handbook                        # 为已有项目生成/更新用户手册
...
使用 Phase 1 提取的需求信息，按照 `.claude/skills/prototype-review/examples/srs.md` 模板生成软件需求规格说明书。

1. **填充项目信息表**：项目名称、文档版本（V1.0）、编制日期（当天）、文档编号（SRS-{年}-{序号}）
2. **第1章 引言**：
   - 编写目的：说明本 SRS 的目标读者和用途
   - 项目背景：从需求文档提取
   - 定义与缩写：提取技术术语
   - 参考资料：列出需求来源文档
3. **第2章 项目概述**：
   - 产品描述：从需求中提取产品定位
   - 功能概览：将 Phase 1 提取的页面清单和功能点表转化为功能域划分
   - 用户特点：从 Phase 1 的角色权限信息提取
   - 运行环境：从非功能性需求提取（或使用合理默认值）
   - 设计与实现限制：从需求中提取技术约束
4. **第3章 功能需求**：
   - 功能划分：生成功能模块表（编号 F-01, F-02, ...），标注优先级 P0/P1/P2
   - 功能详述：对每个功能模块按用例模板展开（用例编号、名称、参与者、前置条件、基本流、后置条件、异常流）
   - 用例模板参见 SRS 示例中的 3.2 节
5. **第4章 非功能需求**：
   - 性能需求：指标 + 目标值表格
   - 安全性需求：鉴权、传输、存储、审计、防护、风控
   - 可靠性需求：可用性、备份、故障恢复
   - 可用性需求、兼容性需求、可维护性需求、可移植性需求
6. **第5章 外部接口需求**：
   - 用户接口（UI 规范）
   - 软件接口（第三方系统对接）
   - 通信接口（协议、鉴权、版本管理）
...
- [ ] **description 语义检查**：每条标注的 description 是否为中文功能说明，而非 CSS 选择器、class 名或 DOM 结构描述？抽查前 5 条，如包含 `.`、`#`、`>` 开头的选择器语法则不合格
...
**description 编写规则（强制——禁止使用 CSS 选择器作为说明）：**

`description` 字段必须是用自然语言书写的**功能说明**，描述元素的作用、交互行为或业务含义。**严禁**将 CSS 选择器、class 名、ID 名、DOM 结构描述作为标注说明。

**格式模板：** `{元素名称/功能}：{交互行为或业务说明}`

**正确示例：**
- "新建任务按钮：点击打开任务创建弹窗，需填写任务名称和执行机器人"
- "地图管理导航项：点击切换到地图管理页面"
- "电量进度条：>60% 绿色，20-60% 黄色，<20% 红色闪烁预警"
- "Tab「进行中」：筛选状态为执行中的任务，空状态显示引导文案"
- "机器人状态卡片：展示当前机器人在线/离线/故障状态，点击查看详情"

**错误示例（禁止）：**
- ~~`.sector .page-tab`~~ — 纯 CSS 选择器，无语义
- ~~`侧边栏 .nav-item`~~ — 混合选择器，描述不完整
- ~~`#addRobotModal 的弹窗`~~ — 以 ID 开头，像定位描述而非功能说明
- ~~`button.btn-primary`~~ — CSS 类名，无功能说明
- ~~`page-section 内的表单`~~ — DOM 结构描述，非功能说明
...
基于需求文档、已生成的 SRS（Phase 1.5）和已完成的原型（Phase 3），按照 `.claude/skills/prototype-review/examples/handbook.md` 模板生成用户手册。

1. **填充产品信息表**：产品名称、版本（V1.0）、手册版本、发布日期、适用读者
2. **第1章 简介**：
   - 关于本手册：说明手册用途和阅读建议
   - 适用读者：从 SRS 用户特点映射（系统管理员、业务主管、普通员工等）
   - 术语说明：从 SRS 术语表提取
   - 阅读约定：提示/注意/警告/快捷键标识说明
3. **第2章 产品概述**：
   - 产品定位：从 SRS 产品描述提炼
   - 主要功能一览表：每个功能模块一句话描述
   - 系统架构简介：简化的分层架构图
4. **第3章 快速上手**：
   ...
   - 登录流程：参照原型的登录页描述操作步骤
   - 首次使用引导：描述新手引导步骤
   - 主界面布局：描述原型的侧边栏 + 主工作区结构（使用 ASCII 图示）
5. **第4章 功能模块说明**：
   - 按原型页面逐个描述功能模块
   - 每个模块包含：操作步骤（编号列表）、字段说明表格（字段/是否必填/说明）
   - Tab 切换、弹窗交互的详细说明
   - 与原型标注（Phase 4）的功能描述保持一致
6. **第5章 FAQ**：基于需求的常见使用场景生成 5-10 个问答
7. **第6章 故障排除**：常见现象→可能原因→解决方案表格
8. **第7章 联系支持**：支持渠道、响应时间
9. **第8章 附录**：快捷键一览、版本更新日志模板

**质量要求**：
- 操作步骤使用编号列表，具体到点击哪个按钮
- 字段说明使用表格格式，与 SRS 保持一致
- FAQ 覆盖登录、权限、数据操作、导出等常见场景
- 故障排除表格至少覆盖 5 个常见问题

保存为 `website/{slug}/handbook.md`。
...
当指定 `--srs`、`--handbook` 或 `--full` 参数时执行：

1. 读取已有项目的 `index.html`、`index.json`、需求文档
2. 如果指定 `--srs`（或 `--full`）：
   - 分析原型页面结构和标注信息
   - 结合需求文档，按照 `.claude/skills/prototype-review/examples/srs.md` 模板生成/更新 SRS
   - 保存为 `website/{slug}/srs.md`
3. 如果指定 `--handbook`（或 `--full`）：
   - 分析原型页面结构和交互逻辑
   - 结合需求和 SRS，按照 `.claude/skills/prototype-review/examples/handbook.md` 模板生成/更新用户手册
   - 保存为 `website/{slug}/handbook.md`
...
website/
  {project-slug}/
    ...
    srs.md          # 软件需求规格说明书（Phase 1.5 生成）
    handbook.md     # 用户手册（Phase 6 生成）
