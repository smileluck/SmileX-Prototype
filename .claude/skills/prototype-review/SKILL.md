...
4. grep 所有可操作按钮：带 onclick 的 button、input[type=submit]
```

**步骤 2：生成标注清单**
按以下顺序逐页生成标注，确保不遗漏：
1. 每个页面的 Tab 切换项（每个 Tab 一个标注）
2. 每个页面内的按钮（创建/新增/导出/搜索/筛选等）
3. 每个页面内的关键数据展示区（卡片/表格/列表）
...
| 元素类型 | 标注内容 | 示例 |
|---------|---------|------|
| **每个可操作按钮** | 功能说明、触发条件、点击后行为 | "新建任务：打开任务创建弹窗，需填写任务名称和执行机器人" |
...
#### D. 标注完整性
- 每个页面是否至少有 2 个标注
- **每个可操作按钮**是否都有标注（新建、编辑、删除、导出、搜索、筛选等）
- **Tab 切换项**是否标注（每个 Tab 的筛选逻辑和内容范围）
- **弹窗内容**是否标注（表单字段、确认/取消按钮、弹窗触发条件）
