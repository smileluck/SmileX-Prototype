...
> Dark-first product design system (core subset — 24 components covering both UI Kits). Tokens authored verbatim from the project root `colors_and_type.css`; component/UI-Kit references derived from the `previews/` HTML pages.
...
├── components/                # Component layer (1 JSON contract per slug, 24 total)
│   ├── index.json
│   ├── activity-rail.json     │ ├── nav-list.json
│   ├── alert.json             │ ├── page-header.json
│   ├── atoms.json             │ ├── pagination.json
...
│   ├── kbd.json               │
│   └── menu.json              │
├── preview/                   # 24 component preview pages (1 HTML per slug — includes `atoms` for shared utilities/atoms)
└── ui_kits/                   # Page-level showcases composing components + tokens
    ├── dashboard/index.html
    └── dev-explorer/index.html
```

> `components.css` is **regenerated** by `design-library-creator/scripts/extract-components-css.mjs`, which scans every `preview/component-*.html` for the CSS between `/* @component-css-start */` and `/* @component-css-end */` markers inside its `<style>` block and aggregates the result. Each preview HTML self-contains its own preview-page chrome (reset, body padding, `.pv-*`) in a `<style>` block above the marker so it never leaks into `components.css`. Shared utilities (`.row`, `.col`, `.grid-*`, `.stack-*`, `.h1/.h2/.h3`, `.eyebrow`, `.text-secondary/.text-tertiary`, `.ds-tooltip`, `.ds-popover`, `.ds-empty`, `.ds-code`, `.mono`/`.num`, `.ds-hero-title`, `.ds-section-title`) live inside `preview/component-atoms.html` and are picked up by the extractor along with the other components. Edit component CSS in the preview HTML, then re-run the script — never edit `components.css` directly.
...
| atoms | Shared atoms | `.row`/`.col`/`.grid-*`/`.stack-*` helpers + `.h1/.h2/.h3`/`.eyebrow` headings + `.ds-tooltip`/`.ds-popover`/`.ds-empty`/`.ds-code` atoms + `.mono`/`.num` tabular utilities + `.ds-hero-title`/`.ds-section-title` |
