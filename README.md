...
2. 生成单文件 HTML 原型（CSS + JS 内联，模拟数据覆盖所有状态），视觉遵循默认设计系统 `design-systems/design-system.md`（冷灰画布 + 白色卡片 + 靛蓝主色的企业数据平台风格）。也可点名其他风格，如"用深色科技风创建原型"；未点名时 AI 会主动询问选择。可选风格见 `design-systems/` 目录：`dark-tech`（深色科技）、`mono-minimal`（极简黑白）、`fresh-emerald`（清新翠绿）、`warm-paper`（暖米纸感）
